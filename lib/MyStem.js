...
function MyStem(args) {
    ...
    this.onlyDictionaryWords = args.onlyDictionaryWords || false;
...
MyStem.prototype = {
    start: function() {
        ...
        var opts = ['--format', 'json', '--eng-gr', '-i'];
        if (this.onlyDictionaryWords)
            opts.push('-w');

        this.mystemProcess = childProcess.spawn(this.path, opts);
